...
- **Atomic Booking Guard**: Since SQLite lacks `EXCLUDE` constraints, `insertBookingIfFree()` performs the overlap check and the INSERT in a single SQL statement — concurrent requests cannot interleave, so no two bookings written through it can overlap. Writes that bypass it entirely are covered by the optional `BOOKINGS_UNIQUE_SLOT_DDL` index.
- **Advisory Locking**: `D1BookingLock` adds fencing-token mutexes to keep contention low and produce friendly errors. It is defence in depth, not the guarantee.
...
// The overlap check and the INSERT are one atomic statement — no lock required
...
See the [D1 API reference](/api-reference/d1/) for buffer time, rescheduling, resource-scoped bookings, and the optional schema-level unique index.
