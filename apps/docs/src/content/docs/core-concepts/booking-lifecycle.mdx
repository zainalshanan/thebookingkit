...
Every booking in SlotKit follows a defined state machine. All transitions are recorded in an append-only audit trail.
...
```typescript
import { getInitialBookingStatus, getAutoRejectDeadline } from "@slotkit/core";
