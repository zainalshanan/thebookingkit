...
SlotKit uses the `rrule` npm package for parsing. The `parseRecurrence()` function expands an RRULE into concrete date occurrences:

```typescript
import { parseRecurrence } from "@slotkit/core";
